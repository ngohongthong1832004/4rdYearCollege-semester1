...
Chạy: python3 train_model_advanced.py
"""
...
os.makedirs("./trained_model_advanced/logs", exist_ok=True)
log_path = f"./trained_model_advanced/logs/train_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class T5WithAdvancedArchitecture(nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None, 
        ...
        if decoder_input_ids is None and labels is not None:
            # HuggingFace utility để shift_right labels -> decoder_input_ids
            decoder_input_ids = self.base_model._shift_right(labels)

        # Forward qua base T5 model
        ...
        if hasattr(outputs, 'encoder_hidden_states') and outputs.encoder_hidden_states is not None:
            ...
            # 4. Ensemble với gating mechanism
...
class CommentGeneratorTrainer:
    ...
    def __init__(self, model_name="VietAI/vit5-base", output_dir="./trained_model_advanced"):
...
def main():
    ...
    trainer = CommentGeneratorTrainer(
        ...
        output_dir="./trained_model_advanced"
    )
