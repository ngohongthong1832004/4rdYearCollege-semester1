"""
Mục đích: Training mô hình T5 với LoRA + Custom Attention + Advanced Techniques
Áp dụng: LSTM, BiLSTM, RNN, Attention Transformer, Ensemble
"""
...
os.makedirs("./trained_model_advanced_v4/logs", exist_ok=True)
log_path = f"./trained_model_advanced_v4/logs/train_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class FeedForwardNetwork(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        x = F.gelu(self.fc1(hidden_states))     # [12, 256, ff_size]   
        x = self.dropout(x)
        x = self.fc2(x)
        x = self.dropout(x)
...
class T5WithAdvancedArchitecture(nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None, 
        ...
        if decoder_input_ids is None and labels is not None:           
            decoder_input_ids = self.base_model._shift_right(labels)   # [12, 64]
...
class CustomTrainer(Trainer):
    ...
    #     """
    #     FIXED: Thêm num_items_in_batch parameter để tương thích với transformers mới
    #     """
    ...
    #     # ✅ Label smoothing: giúp loss mượt hơn, tránh stuck
    #     eps = 0.1
    #     vocab_size = logits.size(-1)
    #     log_probs = F.log_softmax(logits, dim=-1)
    #     labels_flat = labels.view(-1)

    #     # Mask các label -100 (padding)
    #     mask = labels_flat != -100
    #     labels_flat = labels_flat[mask]
    #     log_probs = log_probs.view(-1, vocab_size)[mask]

    #     nll_loss = -log_probs.gather(dim=-1, index=labels_flat.unsqueeze(1)).squeeze(1)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        labels = inputs.get("labels")  # không pop!
        outputs = model(**inputs)
        logits = outputs.logits  # [12, 64, 32004]
        loss_fct = nn.CrossEntropyLoss(ignore_index=-100)
        loss = loss_fct(
            logits.view(-1, logits.size(-1)),       # [12*64, 32004] = [768, 32004]
            labels.view(-1)                         # [12*64] = [768]   
        )
        return (loss, outputs) if return_outputs else loss
...
class CommentGeneratorTrainer:
    ...
    def __init__(self, model_name="VietAI/vit5-base", output_dir="./trained_model_advanced_v4"):
    ...
    def setup_tokenizer(self):
        ...
        print(f"✅ Tokenizer loaded. Vocab: {len(self.tokenizer)} (+{num_added})")
        
        return self.tokenizer
    
    def setup_model(self):
        print("\n" + "="*70)
        print("BƯỚC 2B: SETUP ADVANCED MODEL")
        print("="*70)
    ...
    def tokenize_data(self, train_df, val_df, max_input_length=512, max_target_length=128):
        ...
        print("="*70)
        if os.path.exists("./trained_model_advanced_v4/processed_tokenized/train") and os.path.exists("./trained_model_advanced_v4/processed_tokenized/val"):
            print("📂 Phát hiện tokenized dataset đã tồn tại, đang load lại...")
            tokenized_train = load_from_disk("./trained_model_advanced_v4/processed_tokenized/train")
            tokenized_val = load_from_disk("./trained_model_advanced_v4/processed_tokenized/val")
        ...
        print(f"✅ Train: {len(tokenized_train):,}")
        print(f"✅ Val: {len(tokenized_val):,}")
        os.makedirs("./trained_model_advanced_v4/processed_tokenized", exist_ok=True)
        tokenized_train.save_to_disk("./trained_model_advanced_v4/processed_tokenized/train")
        tokenized_val.save_to_disk("./trained_model_advanced_v4/processed_tokenized/val")
        print("✅ Đã lưu tokenized dataset vào ./trained_model_advanced_v4/processed_tokenized/")          
        return tokenized_train, tokenized_val
...
def main():
    print("""
    ╔══════════════════════════════════════════════════════════════════════╗
    ║     T5 ADVANCED COMMENT GENERATOR - FULL ARCHITECTURE                ║
    ...
    trainer = CommentGeneratorTrainer(
        ...
        output_dir="./trained_model_advanced_v4"
    )
